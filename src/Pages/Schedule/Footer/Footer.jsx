...
const Footer = ({ dates, currentSchedule }) => {
  ...
  const currentSchedulesShifts = shifts.filter(
    (shift) => shift.scheduleId === currentSchedule._id
  );
  ...
  return (
    <div className={`schedule row ${styles.footer}`}>
      <div
        className={`${styles.tile} ${styles.first}`}
      >{`Week: ${totalHours} Scheduled Hrs`}</div>
