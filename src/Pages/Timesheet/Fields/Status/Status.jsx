    ...
    <>
      {timedata.status ? (
        <button
          ...
          onClick={(e) => updateTimecardStatus()}
        >
          <span className={`hide--tablet ${styles.text}`}>
            {!timedata.remove
              ? `${timedata.status.toString().toUpperCase()[0]}${timedata.status
                  .toString()
                  .substring(1)}`
              : "Removed"}
          </span>

          <span className={`show--tablet ${styles.icon}`}>
            {!timedata.remove ? statusIcons() : <BsX />}
          </span>
        </button>
      ) : (
        ""
