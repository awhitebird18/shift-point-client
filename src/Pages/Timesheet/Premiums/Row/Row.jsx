...
// Components
import PremiumSelect from "../PremiumSelect/PremiumSelect";
...
export default function PremiumRow({
  ...
  const { storeTimecard, updatePremiumTime, updatePremiumField } =
  ...
  const premiumDisplay = {
    ...
    hours:
      ...
      Object.prototype.toString.call(premiumData.end) === "[object Date]"
        ? (
            (premiumData.end.getTime() - premiumData.start.getTime()) /
            3600000
          ).toFixed(2) - breakDeductions
        : "",
  };
  ...
  return (
    <tr className={styles.row}>
      ...
      <td className={styles.selects}>
        <PremiumSelect
          premiumId={premiumDisplay.premiumId}
      ...
      <td className={styles.td}>
        {premiumDisplay.status && (
          ...
          >
            {`${
              premiumDisplay.status.toString().toUpperCase()[0]
            }${premiumDisplay.status.toString().substring(1)}`}
          </button>
        )}
      </td>
