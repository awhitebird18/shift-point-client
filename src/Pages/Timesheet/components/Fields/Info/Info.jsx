...
const MoreInfo = ({ timedata, breakdata, date, employee }) => {
  ...
  const { removeTimecard, addTimecard, showModal } = bindActionCreators(actionCreators, dispatch);
  ...
  const iconStyles = `${styles.infoIcon} ${breakdata.length > 0 ? styles.infoIconActive : ""}`;

  const dotStyles = `${styles.dot} ${breakdata.length > 0 ? styles.dotActive : ""}`;
