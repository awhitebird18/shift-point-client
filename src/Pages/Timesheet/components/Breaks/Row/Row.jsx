...
export default function BreaksheetRow({ breaksheetData }) {
  ...
  const { updateBreakField, updateBreakStatus, removeBreak, updateBreakTime } = bindActionCreators(actionCreators, dispatch);
  ...
  const breakIcon = () => {
    ...
    return breaksheetData.length > 0 ? <BsInfoSquareFill style={styles} /> : <BsInfoSquare style={styles} />;
  };
  ...
  return (
    <tr className={`breaks row ${styles.columns}`}>
      <td className={styles.selects}>
        <BreaktypeSelect breakTypeId={breaksheetDisplay.breakTypeId} updateBreakField={updateBreakField} breaksheetData={breaksheetData} />
      </td>
      ...
      <td>
        {breaksheetDisplay.status && (
          ...
          >
            {`${breaksheetDisplay.status.toString().toUpperCase()[0]}${breaksheetDisplay.status.toString().substring(1)}`}
          </button>
        )}
      </td>
