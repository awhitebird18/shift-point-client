...
import { days, months } from "../../functions/timesheetUtils";
...
const TimecardRow = ({ timedata, employee, earningOptions, rowCount }) => {
  ...
  const dateDisplay = `${months[timedata.date.getMonth()]} ${timedata.date.getDate()}`;
  ...
  return (
    <tr className="timecard row columns">
      <td className={styles.date}>
        <p className={styles.dateLarge}>{rowCount === 0 && `${days[timedata.date.getDay()].substring(0, 3)} ${dateDisplay}`}</p>

        <p className={styles.dateSmall}>{rowCount === 0 && `${dateDisplay}`}</p>
      </td>

      <td className={`hide--tablet ${styles.field}`}>
        <PositionField date={timedata.date} timedata={timedata} breakdata={breakdata} employee={employee} timesheetrules={timesheetrules} />
      </td>

      <td className={`hide--mobile ${styles.field}`}>
        <EarningField timedata={timedata} earningOptions={earningOptions} date={timedata.date} employee={employee} />
      </td>

      <td className={`${styles.field}`}>
        <TimeField timedata={timedata} breakdata={breakdata} type={"start"} timesheetrules={timesheetrules} employee={employee} />
      </td>

      <td className={styles.field}>
        <TimeField timedata={timedata} breakdata={breakdata} type={"end"} timesheetrules={timesheetrules} employee={employee} />
      </td>
      ...
      <td className={styles.field}>
        {timecardDisplay.id && <Info timedata={timecardDisplay} breakdata={breakdata} date={timedata.date} employee={employee} />}
      </td>
    </tr>
  );
};
