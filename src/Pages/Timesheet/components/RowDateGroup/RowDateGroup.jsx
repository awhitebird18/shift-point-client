...
const TimecardDate = ({ date, employee, earningOptions }) => {
  ...
  let timedata = useSelector((state) => {
    return state.timedata.timesheet.filter((el) => {
      return el.eeNum === employee.eeNum && el.date.getTime() === date.getTime();
    });
  });
  ...
  return (
    <>
      {filteredData.map((timedata, index) => {
        return <TimecardRow rowCount={index} key={index} timedata={timedata} employee={employee} earningOptions={earningOptions} />;
      })}
    </>
  );
};
