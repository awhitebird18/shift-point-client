...
const TimecardLayout = ({ employee, departments, dateRange, dateRangeHelper, earningOptions }) => {
    ...
    .filter((timecard) => {
      ...
      for (let i = 0; i < timeFilter.length; i++) {
        ...
        if (timeFilter[i].type === "employee") {
          // Missing Punches
          if (timeFilter[i].subtype === "isMissingPunches" && ((timecard.start && timecard.end) || (!timecard.start && !timecard.end))) {
            passed = false;
          }

          if (timeFilter[i].subtype === "isNotMissingPunches" && timecard.start && timecard.end) {
            passed = false;
          }
        ...
        if (timeFilter[i].type === "status") {
          if (timeFilter[i].subtype === "is" && timeFilter[i].value !== timedata.status) {
            passed = false;
          } else if (timeFilter[i].subtype === "isNot" && timeFilter[i].value === timedata.status) {
            passed = false;
          }
        }
      }
  ...
  const approvedHours = calcApprovedHours(filteredTimedata, breakdata, dateRange);

  if (timeFilter.length && filteredTimedata.length <= 1 && !filteredTimedata[0]?.id) return null;
        ...
        <table style={{ width: "100%" }}>
          ...
          <tbody>
            {dateRangeHelper.map((date, index) => {
              return <RowDateGroup key={index} date={date} employee={employee} earningOptions={earningOptions} />;
            })}
          </tbody>
